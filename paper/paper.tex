...
We systematically evaluate five test-time context-compression strategies on 400 matched validation examples: three fixed baselines ($k \in \{3, 5, 10\}$), Adaptive-$k$ similarity-gap truncation~\cite{taguchi2025adaptivek}, UCB1-TUNED rank-based bandit, LinUCB contextual bandit with lexical features, and a proposed \emph{noise-gate} combining cosine-similarity and Jaccard-overlap filtering. The key empirical finding is that all three learned strategies underperform the simple threshold-based noise-gate. We trace each failure to a specific structural cause: uniform gold-document rates across ranks render UCB1-TUNED ineffective, pervasive title non-stationarity at inference time invalidates title-based reward tracking, and semantic indistinguishability between distractors and gold passages defeats lexical-feature regression in LinUCB. Meanwhile, the noise-gate achieves 26.7\% token reduction while staying within $\pm 0.05$~F1 of the $k{=}10$ baseline (F1\,=\,0.764 at $\tau{=}0.3$, $\rho{=}0.65$).

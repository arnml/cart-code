...
%
\begin{abstract}
Retrieval-augmented generation (RAG) commonly relies on fixed top-$k$ retrieval, which is simple but token-intensive. On HotpotQA distractor, fixed $k=10$ retrieval achieves strong F1 (0.792 on GPT-5.4-mini) while using about $14\times$ more tokens than no retrieval (1452.5 vs 103.5). We study five test-time context-compression strategies on this benchmark: fixed baselines ($k$=3, 5, 10), Adaptive-$k$ similarity-gap truncation, UCB1-TUNED rank-based bandit learning, LinUCB contextual bandit with lexical features, and a noise-gate method combining cosine-similarity and Jaccard-overlap filtering.

On a matched sample of 400 HotpotQA distractor validation examples with GPT-5.4-mini, the noise gate at cosine threshold $\tau=0.3$ offers the best quality-cost tradeoff: F1=0.764 with 26.7\% fewer tokens than $k=10$ (1065.3 vs 1452.5), while staying within $\pm$0.05 F1 of the baseline. Adaptive-$k$ produces compact contexts ($k^*=3.02$ on average) but reaches F1=0.627; UCB1-TUNED at $k=5$ reaches F1=0.704 yet falls back to BM25 ranking on 83.8\% of queries; and LinUCB at $k=5$ reaches F1=0.620, showing that lexical features alone are not enough for this benchmark.

These results are explained by three structural properties of distractor-heavy multi-hop QA: near position-independent gold-document rates, high title non-stationarity between training and inference, and semantic closeness between distractors and true evidence. In this setting, embedding-based threshold filtering is a strong and interpretable test-time controller that achieves the best Pareto point on the quality--cost frontier.
...
\begin{enumerate}
    ...
    \item \textbf{Noise-Gate method}: achieves $-$26.7\% token reduction while staying within $\pm 0.05$~F1 of the $k{=}10$ baseline (F1\,=\,0.764 vs.\ 0.792 at $\tau{=}0.3$, $\rho{=}0.65$).

...
On a matched sample of 400 HotpotQA distractor validation examples with GPT-5.4-mini, the noise gate with cosine threshold $\tau=0.3$ achieves F1=0.764 while reducing total tokens by 26.7\% relative to $k=10$ (1065.3 vs 1452.5 tokens), remaining within $\pm$0.05 F1 of the baseline. In contrast, Adaptive-$k$ selects only $k^*=3.02$ documents on average but reaches only F1=0.627; UCB1-TUNED at $k=5$ (F1=0.704) appears competitive with fixed $k=5$ (0.716) but silently falls back to BM25 ranking on 83.8\% of queries; and LinUCB at $k=5$ (F1=0.620) cannot distinguish semantically deceptive distractors using lexical features alone.

"""Analyze and summarize 2WikiMultiHopQA baseline results.

Reads CSV from ``experiments/2wiki/results/baseline/`` and writes a
dataset-local markdown summary next to it.

Usage from root:
    uv run python -m experiments.2wiki.analyse_baseline gpt-5.4-mini
"""
...
def load_csv(model: str) -> tuple[list[dict[str, str]], Path]:
    csv_path = RESULTS_DIR / f"baseline_{model}_2wiki_validation.csv"
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Results file not found: {csv_path}\n"
            f"Run: uv run python -m experiments.2wiki.run_baseline <n_rows> [max_workers]"
        )
...
def generate_summary(model: str, aggregated: dict[str, dict[str, float]]) -> str:
    lines = [
        f"# 2Wiki Baseline Analysis: {model}\n",
        "## Summary by Method\n",
...
def save_summary(model: str, summary: str) -> None:
    md_path = RESULTS_DIR / f"analysis_{model}_2wiki_validation.md"
    md_path.write_text(summary, encoding="utf-8")
    print(f"Summary saved: {md_path}")


def analyse_baseline(model: str) -> None:
    print(f"Analyzing 2Wiki baseline results for: {model}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: uv run python -m experiments.2wiki.analyse_baseline <model>")
        print("\nExample:")
        print("  uv run python -m experiments.2wiki.analyse_baseline gpt-5.4-mini")
        sys.exit(1)

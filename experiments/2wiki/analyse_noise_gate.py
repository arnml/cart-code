"""Analyze and summarize 2WikiMultiHopQA noise-gate results.

Reads CSV from ``experiments/2wiki/results/noise_gate/`` and produces a
summary report with aggregated metrics by threshold.

Usage from root:
    uv run python -m experiments.2wiki.analyse_noise_gate gpt-5.4-mini
    uv run python -m experiments.2wiki.analyse_noise_gate gpt-5.4-mini validation
"""
...
def load_csv(model: str, split: str) -> tuple[list[dict[str, str]], Path]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            ...
            f"Run: uv run python -m experiments.2wiki.run_noise_gate {model} <n_rows> [max_workers]"
        )
...
def generate_summary(model: str, split: str, aggregated: dict[float, dict[str, float]]) -> str:
    ...
    lines = [
        f"# 2Wiki Noise-Gate Analysis: {model} ({split})\n",
        "| " + " | ".join(header_cells) + " |",
...
def analyse_noise_gate(model: str, split: str = "validation") -> None:
    ...
    print(f"Analyzing 2Wiki noise-gate results for: {model} ({split})")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: uv run python -m experiments.2wiki.analyse_noise_gate <model> [split]")
        print("\nExample:")
        print("  uv run python -m experiments.2wiki.analyse_noise_gate gpt-5.4-mini")
        print("  uv run python -m experiments.2wiki.analyse_noise_gate gpt-5.4-mini validation")
        sys.exit(1)

"""MuSiQue-Ans noise-gate evaluation.

Usage from root:
    uv run python -m experiments.musique_ans.run_noise_gate 10
    uv run python -m experiments.musique_ans.run_noise_gate 10 5
    uv run python -m experiments.musique_ans.run_noise_gate 10 5 --split validation
"""
...
def _evaluate_prediction(prediction: str, sample: dict[str, Any]):
...
def run_noise_gate(
    ...
    print("\n" + "=" * 70)
    print(
        f"Running MuSiQue-Ans noise-gate: {model} (n={n_rows}, workers={max_workers}, split={split})"
    )
    print(
        "Thresholds: "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the MuSiQue-Ans noise gate.")

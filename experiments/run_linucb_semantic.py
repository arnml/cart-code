"""LinUCB-Semantic reranker evaluation script with embeddings.

Loads a trained LinUCB-Semantic model and evaluates on HotpotQA validation split.
Tests multiple k values (2, 3, 5) in a single run, producing one CSV row per k per question.

Usage from root:
    uv run python -m experiments.run_linucb_semantic gpt-4o-mini 100
    uv run python -m experiments.run_linucb_semantic claude-sonnet-4-6 100 20

Requires:
    - Trained model: experiments/cache/linucb_semantic_model.json
    - Val embeddings: experiments/cache/val_embeddings.npz
"""
...
RESULTS_DIR = Path(__file__).parent / "results" / "linucb_semantic"
RESULTS_DIR.mkdir(exist_ok=True, parents=True)

MODEL_PATH = Path(__file__).parent / "cache" / "linucb_semantic_model.json"
EMBEDDINGS_PATH = Path(__file__).parent / "cache" / "val_embeddings.npz"
LINUCB_K_VALUES = (2, 3, 5)
LINUCB_METHOD = "linucb_semantic"
...
def save_csv(model: str, results: list[ResultDict]) -> None:
    ...
    csv_path = RESULTS_DIR / f"linucb_semantic_{model}.csv"
...
def _run_linucb_semantic_inference(
    sample: dict[str, Any],
    sample_idx: int,
    model: str,
    reranker: LinUCBSemantic,
    embeddings: dict[str, Any],
    k: int,
) -> tuple[str, int, int, float, list[str]]:
    """Run LinUCB-Semantic selection and LLM generation for one k value."""
    ...
    q_embedding = embeddings["questions"].get(sample_idx)
    if q_embedding is None:
        raise KeyError(f"Question embedding missing for sample {sample_idx}")

    # Collect section embeddings by rank
    s_embeddings = {}
    for rank in range(len(titles)):
        s_emb = embeddings["sections"].get((sample_idx, rank))
        if s_emb is None:
            raise KeyError(f"Section embedding missing for ({sample_idx}, {rank})")
        s_embeddings[rank] = s_emb

    # Select top-k by LinUCB-Semantic score
    selected_titles = reranker.select(
        question,
        titles,
        sentences_list,
        k,
    ...
    for k in LINUCB_K_VALUES:
        try:
            (
                ...
                cost_usd,
                selected_titles,
            ) = _run_linucb_semantic_inference(
                sample, sample_idx, model, reranker, embeddings, k
            )
...
def run_linucb_semantic(
    model: str,
    n_rows: int,
    max_workers: int = DEFAULT_MAX_WORKERS,
) -> None:
    """Run the LinUCB-Semantic reranker evaluation for a model."""
    if max_workers < 1:
        raise ValueError("max_workers must be >= 1")

    print("\n" + "=" * 70)
    print(f"Running LinUCB-Semantic: {model} (n={n_rows}, workers={max_workers})")
    ...
    if not EMBEDDINGS_PATH.exists():
        ...
        print("Please run: uv run python -m experiments.precompute_embeddings validation <output_path>")
        sys.exit(1)
    ...
    print(f"Loaded {len(embeddings['questions'])} question embeddings")
    print(f"Loaded {len(embeddings['sections'])} section embeddings")

    # Load model
    print("\nLoading LinUCB-Semantic model...")
    if not MODEL_PATH.exists():
        print(f"Error: Model not found at {MODEL_PATH}")
        print("Please train the reranker first:")
        print("  uv run python -m experiments.train_linucb_semantic")
        sys.exit(1)
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        futures = [
            executor.submit(_process_sample, i, sample, model, reranker, embeddings)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: uv run python -m experiments.run_linucb_semantic "
    ...
    run_linucb_semantic(model, n_rows, max_workers=max_workers)
